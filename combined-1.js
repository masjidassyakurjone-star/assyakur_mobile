/* ==========================================================================
   COMBINED.JS - ENGINE SIGNAGE MASJID ASSYAKUR V2.8 (MOBILE + SHALAWAT PRE-ADZAN)
   ========================================================================== */
...
function putarRangkaianAdzan('shalawat.mp3');
    ...
    audioShalawat.play().then(() => {
        audioShalawat.onended = () => {
            console.log("Shalawat selesai. Memicu Beep Pendek...");
            const audioBeepPendek = new Audio('BEEP PENDEK.mp3');
            
            audioBeepPendek.play().then(() => {
                audioBeepPendek.onended = () => {
                    console.log("Memicu Beep Panjang tanda masuk waktu adzan...");
                    const audioBeepPanjang = new Audio('BEEP PANJANG.mp3');
                    audioBeepPanjang.play().catch(e => console.error("Gagal Beep Panjang:", e));
                };
            }).catch(e => console.error("Gagal Beep Pendek:", e));
        };
    }).catch(e => console.error("Gagal memutar Shalawat (Pastikan sudah interaksi double click di layar):", e));
}

function triggerAlarm(tipe) {
    if (tipe === 'iqamah') {
        console.log("Memicu jalannya alarm 7 detik sebelum Iqamah...");
        const audio = new Audio('BEEP PENDEK.mp3');
...
setInterval(() => {
    ...
    // CRITICAL FIX: Trigger tepat 290 detik (4 menit 50 detik) sebelum adzan masuk waktu
    if (sisaDetik === 290 && !sholatBerikutnya.isBesok && !isAudioShalawatPlay) {
        isAudioShalawatPlay = true;
        putarRangkaianAdzan('shalawat.mp3'); // Pastikan file audio shalawat Anda diberi nama shalawat.mp3
        setTimeout(() => { isAudioShalawatPlay = false; }, 15000);
    }
    ...
    for (let i = 0; i < daftarSholat.length; i++) {
        ...
        if (sekarangDetik >= adzanDetik && sekarangDetik < adzanDetik + batasIqamahDetik) {
            iqamahAktif = {
                ...
                sisaDetik: (adzanDetik + batasIqamahDetik) - discreditedDetik = (adzanDetik + batasIqamahDetik) - sekarangDetik
            };
            break;
        }
    }
